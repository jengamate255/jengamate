...
void main() {
  ...
  setUp(() {
    ...
    when(mockNotificationsCollection.doc(any)).thenReturn(mockNotificationDocument);
    when(mockNotificationsCollection.where(any, isEqualTo: anyNamed('isEqualTo'))).thenReturn(mockQuery);
  ...
  group('DatabaseService Tests', () {
    ...
    group('getUser', () {
      ...
      test('should return null on Firestore exception', () async {
        // Arrange
        when(mockUserDocument.get()).thenThrow(FirebaseException(plugin: 'firestore'));
    ...
    group('createUser', () {
      ...
      test('should throw exception on failure', () async {
        ...
        // Act & Assert
        expect(() => databaseService.createUser(testUser), throwsA(isA<FirebaseException>()));
      });
    });
    ...
    group('updateUser', () {
      ...
      test('should throw exception on failure', () async {
        ...
        // Act & Assert
        expect(() => databaseService.updateUser(testUser), throwsA(isA<FirebaseException>()));
      });
    });
    ...
    group('createNotification', () {
      ...
      test('should complete successfully when notification is created', () async {
        // Arrange
        when(mockNotificationDocument.set(any)).thenAnswer((_) async => Future.value());
        ...
        verify(mockNotificationDocument.set(testNotification.toMap())).called(1);
      });
      ...
      test('should throw exception on failure', () async {
        ...
        // Act & Assert
        expect(() => databaseService.createNotification(testNotification), throwsA(isA<Exception>()));
      });
    });
    ...
    group('streamUserNotifications', () {
      test('should return a stream of notifications for a user', () {
        ...
        final mockNotificationDocSnapshot = MockQueryDocumentSnapshot<Map<String, dynamic>>();
        when(mockNotificationDocSnapshot.data()).thenReturn(notification.toMap());
